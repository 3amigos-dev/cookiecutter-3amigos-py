...
for PYVER in ${PYTHONVERS} ; do
  ...
  PYTEST_FAIL="NO"
  if ! "python${PYVER}" -m pytest -n auto --cov-config=.coveragerc --cov-fail-under={{ cookiecutter.cov_fail_under }} "--cov=${MAIN_MODULE}" --cov-report=xml:test-cov.xml --cov-report=html --cov-report=term-missing ; then
    PYTEST_FAIL="YES"
  fi
