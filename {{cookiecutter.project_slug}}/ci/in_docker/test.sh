...
for PYVER in ${PYTHONVERS} ; do
  ...
  PYTEST_FAIL="NO"
  if ! "python${PYVER}" -m pytest --cov-config=.coveragerc --cov-fail-under={{ cookiecutter.cov_fail_under }} "--cov=${MAIN_MODULE}" --cov-report=xml:test-cov.xml --cov-report=html ; then
    PYTEST_FAIL="YES"
  fi
