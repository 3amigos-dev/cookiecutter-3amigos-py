...
for PYVER in ${PYTHONVERS} ; do
  ...
  "python${PYVER}" -m pytest -n auto --cov-config=.coveragerc --cov-fail-under={{ cookiecutter.cov_fail_under }} "--cov=${MAIN_MODULE}" --cov-report=xml:test-cov.xml --cov-report=html
