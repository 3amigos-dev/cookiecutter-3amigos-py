...
LATESTPYVER={{ cookiecutter.latest_python }}
PYVER="${LATESTPYVER}"
# Run spelling in root to check docs
"python${PYVER}" -m spelling

# Run in {{ cookiecutter.docker_application_dirname }} for python checks
cd "${BASEDIR}/{{ cookiecutter.docker_application_dirname }}"
# Run black to check all python on {{ cookiecutter.latest_python }} only
"python${PYVER}" -m black --check --diff "${BASEDIR}"
# Run pylint to lint all python on {{ cookiecutter.latest_python }} only
